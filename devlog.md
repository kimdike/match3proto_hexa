...
### 1. 스킨 저장 흐름 — 명시 저장 버튼 + 모달 (나가기/저장 + ✕)
- 슬롯 변경 즉시 저장 X → in-memory만 갱신 + dirty 표시
- 헤더 우측 "저장" 버튼 (dirty 시 노란 활성, 비-dirty 시 비활성)
- 뒤로가기(←) 시 dirty면 모달: **나가기**(폐기) / **저장** + 우상단 **✕**(취소)
- 첫 버전은 "폐기/취소/저장 3버튼"이었으나 사용자 요청으로 2버튼 + ✕ 단순화

### 2. 천장 게이지 위치 — ADVENTURE START 버튼 안 통합
- 헤더 메타 row에서 빠진 후 별도 element로 입장 버튼 위에 있던 거 → 버튼 안 우상단 배지로 통합
- `⭐ 0/5` 작은 알약. 5/5 도달 시 노란 그라디언트 + pulse (`is-full` 클래스 연동)
- 헤더 공간 확보 + 시각 통합 (입장 직전에 자연스럽게 보임)

### 3. 하트 소비 룰 변경 — 입장 X, 결과 분기
| 결과 | 하트 소비 |
|---|---|
| 클리어 | X |
| 패배 (Move 0) | −1 |
| 나가기 (STOP confirm) | −1 |
- 입장 시 검사만 (0이면 모달), 소비는 게임 결과 시점
- `showEndScreen(false)` 분기에 consumeHeart 호출 + `confirm-yes` 핸들러(STOP 확인)에도 추가
- 충전 주기: **프로토 1분** (`HEART_INTERVAL_MS = 60 * 1000`) — 출시 시 10분 복귀 필요

### 4. 상점 buy 탭 — 더미 구매 활성화
- 옛 placeholder 폐기. 카드 3종:
  - 💎 다이아 +1 (결제 X, 즉시 +1)
  - 🍬 사탕 +10 (즉시 +10)
  - 🛒 몬스터볼 (골드) — 가격 밸런싱 후 활성 placeholder
- 다이아 0 상태에서 하트 부족 모달 → "💎 구매하러 가기" → 상점 buy 탭 자동 이동

### 💡 오늘의 교훈
1. **즉시 저장 vs 명시 저장의 UX 차이**: 사용자가 슬롯 실수 변경했을 때 되돌릴 수 있어야 함. dirty + 저장 버튼 + 모달 패턴은 map_editor에서 검증된 좋은 흐름. 스킨에도 동일 적용.
2. **재화 모달의 "구매 유도" UX**: 다이아 0이면 충전 버튼 disabled로 막는 것보다, 활성으로 두고 클릭 시 자연스럽게 구매 화면으로 안내가 더 좋음 (사용자 피드백). "도구가 막혀있는" 느낌이 아니라 "이렇게 얻을 수 있다"는 흐름.
3. **하트 소비 시점은 게임 결과에 묶이는 게 자연스러움**: 입장 시 -1보다, "실패했으니 -1" 또는 "포기했으니 -1"이 사용자 인지에 자연스럽다 (사용자 피드백). 클리어는 보상이라 소비 X.
4. **프로토용 짧은 충전 주기**: 검증 사이클 빠르게. 출시 시 환원 잊지 말 것 (코드 주석으로 기록).

---

## 2026.05.10 (19일차 마지막) — 묶음 1·2·3 일괄 구현 (재료/상점 + 메타 재화 + 포켓몬 성장)

사용자 5시간 부재 동안 1~3 묶음 자동 구현. ralph는 비추 + 일반 implementation. 임의 밸런스값은 추후 사용자 조정 가능.

### 묶음 1 — 재료 시스템 + 상점 + 볼 부족 다이얼로그
- `materials.js` 신규 — 3종 재료(기본/슈퍼/하이퍼), 60/30/10% 드롭, 5개 합성 → 볼 1
- main.js `onStageCleared` 부활 — 클리어 시 `dropRandomMaterial()` 호출
- ui.js 클리어 화면 — 재료 획득 라인 추가 (`_lastClearReward`)
- index.html `#shop-screen` 신규 + lobby `🛒 상점` 버튼 활성
- 상점 화면 — 합성 탭 (3종 카드 + 진행률 바 + 합성 버튼) / 골드 구매 탭 placeholder
- 가방 재료 탭 — 사탕 + 3종 재료 카드 (CSS 결정 도형 아이콘)
- 조우 화면 볼 부족 다이얼로그 — 던지기 버튼 활성화 유지 + 클릭 시 모달 → 상점 이동
- localStorage `hexPuzzleMaterials` + clear 키 추가

### 묶음 2 — 메타게임 재화 (하트 + 다이아 + 덱 보너스)
- `hearts.js` 신규 — basic 자연충전 패턴 그대로 (Math.floor 사용, 비트 버그 회피)
  - 5/5 디폴트, 10분/+1, 5+이면 정지, 5에서 4로 떨어지는 순간 chargeAt = now
  - `consumeHeart` / `addHearts` / `refillHeartsByDiamond` (다이아 1 → 하트 풀)
- 로비 헤더 하트 카드 (`❤️ N/5 +mm:ss`) — 핑크/빨강 톤
- main.js startGame 진입 흐름 — `lobby-stage-btn` 클릭 시 하트 검사 + 소비
- 하트 부족 모달 — 다이아 충전 / 취소 (다이아 부족 시 충전 버튼 disabled)
- main.js `onStageCleared` — 다이아 5% 확률 +1 (`_lastClearDiamond`)
- main.js `getDeckTypeBonus` — 지역 타입과 같은 타입 마리수 → 골드 배율 (3/4/5/6 = 1.3/1.5/1.7/2.0)
- ui.js showEndScreen — 골드 × 보너스 적용 + 클리어 화면 인라인 배지 표시
- localStorage `hexPuzzleHeart` / `hexPuzzleHeartChargeAt` + clear 키 추가

### 묶음 3 — 포켓몬 성장 (사탕 진화 + 크기/무게)
- dex.js `evolveMonster(id, targetId)` — 사탕 차감 + `captureNow(target)` + 보너스 +1
- dex.js `getEvolutionInfo` — 진화 가능 여부 + 비용 + 분기/단순 판정
- 도감 상세 모달에 진화 섹션 동적 렌더 (`renderEvolutionSection`):
  - 단순 진화: 진화체 sprite + "진화하기" 버튼
  - 분기 진화 (이브이): 3 버튼 그리드 (샤미드/쥬피썬더/부스터)
  - 사탕 부족 시 "X개 부족" 안내
- encounter.js 포획 성공 시 height/weight 랜덤 (factor 0.7~1.3)
  - XXS (factor < 0.78) / XXL (factor > 1.22) 라벨 산출
  - `captureNow(id, {height, weight})` — dex.js의 biggest/smallest 자동 갱신
  - 결과 모달에 ⭐ XXS/XXL 배지 + 키/무게 정보 표시

### 임의 밸런스값 (추후 사용자 조정)
- 합성 골드 비용: **무료** (밸런싱 후속)
- 하트 시작: **5/5** Full
- 하트 충전 주기: **10분/+1**, Max 5
- 다이아 시작: **0**, 클리어 시 **5% 확률 +1**
- 다이아 → 하트 충전 비용: **다이아 1 → 하트 5 (Full)**
- 덱 타입 보너스: 같은 타입 **3/4/5/6 → 1.3/1.5/1.7/2.0배** 골드
- 크기/무게 factor: **0.7~1.3 균등 랜덤**
- XXS/XXL 임계: **factor < 0.78 / > 1.22** (각 약 13% 확률)
- 진화 보너스: 사탕 **+1**
- 재료 드롭: **기본 60% / 슈퍼 30% / 하이퍼 10%** (마스터 X — 합성 불가)

### ⚠️ 자동 commit 안 함 (사용자 메모리 준수)
사용자 검증 후 직접 commit/push.
