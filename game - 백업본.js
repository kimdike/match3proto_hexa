...
function findAllMatches(){
  ...
  // 4+ 동색 인접 클러스터 감지 (BFS) — 타겟볼/폭탄볼 등 비직선 매치용
  const clVisited=new Set();
  let hasCluster4=false;
  for(let col=0;col<COLS_PATTERN.length;col++){
    for(let row=0;row<COLS_PATTERN[col];row++){
      const k=`${col},${row}`;
      ...
      if(group.length>=4){
        console.log(`[클러스터] 감지! 색:${color}, 크기:${group.length}, 셀:`,group.map(([c,r])=>`${c},${r}`));
        hasCluster4=true;
        group.forEach(([c,r])=>cellSet.add(`${c},${r}`));
      }
    }
  }

  return {lines,cells:[...cellSet].map(k=>k.split(',').map(Number)),hasCluster4};
}
...
function findConnectedGroups(matchedCells){
  ...
  for(const [color,cells] of Object.entries(byColor)){
    ...
    for(const [sc,sr] of cells){
      ...
      // 직선 판정
      const isStraight=AXES.some(([dirA])=>{
        for(const [startC,startR] of group){
          const ls=new Set(); ls.add(`${startC},${startR}`);
          let pos=step(startC,startR,dirA);
          while(pos&&keySet.has(`${pos[0]},${pos[1]}`)){ls.add(`${pos[0]},${pos[1]}`);pos=step(pos[0],pos[1],dirA);}
          const [,dirB]=AXES.find(([a])=>a===dirA);
          pos=step(startC,startR,dirB);
          while(pos&&keySet.has(`${pos[0]},${pos[1]}`)){ls.add(`${pos[0]},${pos[1]}`);pos=step(pos[0],pos[1],dirB);}
          if(group.every(([c,r])=>ls.has(`${c},${r}`))) return true;
        }
        return false;
      });
      results.push({color:parseInt(color),cells:group,isStraight,size:group.length});
    }
  }
  return results;
}
...
function determineSpecial(curLines,curCells,isFirst,originCol,originRow,destCol,destRow,swapDir){
  // 1) 후보 라인을 찾는다
  ...
  // 2) 연결 그룹
  const groups=findConnectedGroups(curCells);
  let bentGroup5=null,bentGroup4=null;
  for(const g of groups){
    if(g.size>=5 && !g.isStraight && !bentGroup5) bentGroup5=g;
    if(g.size>=4 && !g.isStraight && !bentGroup4) bentGroup4=g;
  }
  ...
  // 2순위: 폭탄볼 (비직선5+)
  if(bentGroup5){
    const pivot=choosePivot(bentGroup5.cells);
    return {type:'bomb',col:pivot.col,row:pivot.row,color:bentGroup5.color,dir:null,mergeCells:bentGroup5.cells};
  }
  ...
  // 4순위: 타겟볼 (4개 연결 + 비직선)
  if(bentGroup4){
    const pivot=choosePivot(bentGroup4.cells);
    return {type:'target',col:pivot.col,row:pivot.row,color:bentGroup4.color,dir:null,mergeCells:bentGroup4.cells};
  }
  ...
  for(let col=0;col<COLS_PATTERN.length;col++)
    for(let row=0;row<COLS_PATTERN[col];row++){
      ...
      for(const [nc,nr] of getNeighbors(col,row)){
        ...
        const {lines,hasCluster4}=findAllMatches();
        let mx=0; for(const l of lines) if(l.length>mx) mx=l.length;
        if(hasCluster4&&mx<4) mx=4;
        [board[col][row],board[nc][nr]]=[board[nc][nr],board[col][row]];
...
function onDragEnd(e){
  ...
  if(target) trySwap(col,row,target[0],target[1]);
}
...
async function trySwap(c1,r1,c2,r2){
  ...
  if(sp1&&sp2){
    ...
    let {lines:cl,cells:cc}=findAllMatches();
    let combo=0;
    while(cc.length>0){
      combo++;
      await processMatchStep(cl,cc,false,c1,r1,c2,r2,null,combo);
      await applyGravity();await fillEmpty();
      const chain=findAllMatches();cl=chain.lines;cc=chain.cells;
    }
    checkGameEnd();busy=false;startHintTimer();return;
  }
  ...
  if(rb1||rb2){
    ...
    if(targetColor!==null&&targetColor!==undefined){
      movesLeft--;updateMovesUI();
      const cnt=await activateRainbow(rainbowPos.col,rainbowPos.row,targetColor);
    ...
    swapBoard(c1,r1,c2,r2);await animateSwap(c1,r1,c2,r2);
    busy=false;startHintTimer();return;
  }

  const {lines,cells}=findAllMatches();
  if(cells.length===0){
    swapBoard(c1,r1,c2,r2);await animateSwap(c1,r1,c2,r2);
    busy=false;startHintTimer();return;
  }
  ...
  let combo=0,curLines=lines,curCells=cells,isFirst=true;
  while(curCells.length>0){
    combo++;
    await processMatchStep(curLines,curCells,isFirst,c1,r1,c2,r2,swapDir,combo);
    isFirst=false;
    await applyGravity();await fillEmpty();
    const chain=findAllMatches();curLines=chain.lines;curCells=chain.cells;
  }
  checkGameEnd();busy=false;startHintTimer();
}
...
// ── 매치 1단계 처리 ──
async function processMatchStep(curLines,curCells,isFirst,originCol,originRow,destCol,destRow,swapDir,combo){
  // 1) 특수볼 생성 판정
  const specialInfo=determineSpecial(curLines,curCells,isFirst,originCol,originRow,destCol,destRow,swapDir);
  ...
  // 매치에 포함된 무지개볼은 이미 actRainbows에 있으므로, extra에서 새로 발견된 것만 추가 처리
    ...
    for(let row=COLS_PATTERN[col]-1;row>=0;row--){
      if(board[col][row]!==null){
        if(row!==wr){
          ...
          const el=blockEls[col][wr]; el.dataset.row=wr;
...
async function processPendingMatches(){
  if(!playing) return;
  let {lines,cells} = findAllMatches();
  if(cells.length===0) return;
  let combo=0;
  while(cells.length>0){
    combo++;
    await processMatchStep(lines,cells,false,-1,-1,-1,-1,null,combo);
    await applyGravity();
    await fillEmpty();
    const next = findAllMatches();
    lines=next.lines; cells=next.cells;
  }
  checkGameEnd();
}
...
function resetToStart(){
  ...
  playing=false;busy=false;dragState=null;
  debugPlaceType=null;
